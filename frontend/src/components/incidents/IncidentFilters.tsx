...
import { IncidentFilterParams } from '../../api/incidents';

interface IncidentFiltersProps {
  filters: IncidentFilterParams;
  onApplyFilters: (filters: IncidentFilterParams) => void;
  onClearFilters: () => void;
  isLoading?: boolean;
}
...
export const IncidentFilters: React.FC<IncidentFiltersProps> = ({
  ...
  const [localFilters, setLocalFilters] = useState<IncidentFilterParams>(filters);
  const [isExpanded, setIsExpanded] = useState(false);

  const handleChange = (key: keyof IncidentFilterParams, value: string) => {
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/90 p-4 shadow-sm backdrop-blur-xs space-y-3">
      ...
      <form onSubmit={handleApply} className="space-y-3">
        ...
        {/* Advanced Filters */}
        {isExpanded && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 border-t border-slate-800 pt-3">
            <div>
              <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">
                From Date
              </label>
